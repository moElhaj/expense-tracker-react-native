...
export default function New({ setScreen, getExpenses }) {
	const { user } = useAuth();
	...
	const addExpense = async () => {
		...
		if (isNaN(parseFloat(amount))) {
			return alert("Amount should be a number");
		}
		...
		try {
			...
			setError("Error adding expense, try again later");
		}
		setLoading(false);
		setScreen("home");
	};
			...
			>
				<View style={styles.container}>
					...
					<View style={{ paddingTop: 10 }}>
						...
						<View style={{ paddingTop: 10 }}>
							<Button onPress={addExpense} title="Add" color="#4f46e5" />
						</View>
					</View>
				</View>
			</KeyboardAvoidingView>
